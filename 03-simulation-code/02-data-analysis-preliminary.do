...
postfile pf double(i model scenario parameter b stderr converged error) str20(name) using "simulation/data/02-preliminary-results-ci.dta", replace
...
forvalues s = 1/`scenarios' {
	...
	forvalues i = 1/`B' {
		quietly {
			use "simulation/data/simdata/01-preliminary-ci-`s'-`i'", clear
			* use "simulation/data/simdata/01-preliminary-ci-1-1", clear
			compress
			save, replace
			// LMM
			capture gsem ///
...
use "simulation/data/02-preliminary-results-ci.dta", replace
compress
save, replace
...
// Create postfile
capture postclose pf
postfile pf double(i model scenario parameter b stderr converged error) str20(name) using "simulation/data/02-preliminary-results-gi.dta", replace
...
forvalues s = 1/`scenarios' {
	...
	forvalues i = 1/`B' {
		quietly {
			use "simulation/data/simdata/01-preliminary-gi-`s'-`i'", clear
			* use "simulation/data/simdata/01-preliminary-gi-1-1", clear
			compress
			save, replace
			// LMM
			capture gsem ///
...
// Compress to get a smaller file
use "simulation/data/02-preliminary-results-gi.dta", replace
compress
save, replace
