...
postfile pf double(i model scenario parameter b stderr converged error) str20(name) using "simulation/data/05-full-results-ci.dta", replace
...
forvalues s = 1/`scenarios' {
	...
	forvalues i = 1/`B' {
		quietly {
			use "simulation/data/simdata/04-full-ci-`s'-`i'", clear
			* use "simulation/data/simdata/04-full-ci-1-1", clear
			compress
			save, replace
			// LMM
			capture gsem ///
...
use "simulation/data/05-full-results-ci.dta", replace
compress
save, replace
...
// Create postfile
capture postclose pf
postfile pf double(i model scenario parameter b stderr converged error) str20(name) using "simulation/data/05-full-results-gi.dta", replace
...
forvalues s = 1/`scenarios' {
	...
	forvalues i = 1/`B' {
		quietly {
			use "simulation/data/simdata/04-full-gi-`s'-`i'", clear
			* use "simulation/data/simdata/04-full-gi-1-1", clear
			compress
			save, replace
			// LMM
			capture gsem ///
...
// Compress to get a smaller file
use "simulation/data/05-full-results-gi.dta", replace
compress
save, replace
